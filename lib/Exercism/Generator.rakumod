...
has %.cdata = do if ( my $cdata-file =
  ( (.IO with %*ENV<XDG_CACHE_HOME>) // Empty, |('.cache', <Library Caches>).map({%*ENV<HOME>.IO.add: $_}) )
  .map({.add("exercism/configlet/problem-specifications/exercises/$!exercise/canonical-data.json")})
  .first(*.f)
) {
